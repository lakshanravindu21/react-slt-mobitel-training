...
const JobModification = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f9fbfc] text-base md:text-lg">
      {/* Header */}
      <header className="bg-white shadow px-6 py-4 flex justify-between items-center rounded-b-3xl border-b text-sm md:text-base">
      ...
      <div className="flex flex-1 flex-col md:flex-row">
        {/* Sidebar */}
        <aside className="bg-[#0052a0] w-full md:w-56 py-8 px-4 text-white">
        ...
        {/* Main Content */}
        <section className="flex-1 p-4 md:p-6 relative flex flex-col items-center w-full">
          {/* Search Box */}
          <div className="absolute top-6 right-6 bg-white border border-gray-300 rounded-full shadow-inner flex items-center px-4 py-1 text-sm">
            <input
              type="text"
              placeholder="Search"
              className="outline-none border-none px-2 py-1 w-32 md:w-44 text-base"
            />
            <button>
          ...
          <div className="w-full max-w-6xl bg-white rounded-2xl shadow-md p-6 md:p-8 mt-24 hover:scale-[1.01] transition-transform duration-300">
                ...
                <tbody>
                  {jobs.map((job, index) => (
                    <tr key={index} className={index % 2 === 0 ? "bg-[#f5f7fa]" : "bg-white"}>
                      <td className="px-4 md:px-6 py-4">{job.id}</td>
                      <td className="px-4 md:px-6 py-4">{job.field}</td>
                      <td className="px-4 md:px-6 py-4">{job.date}</td>
                      <td className="px-4 md:px-6 py-4">{job.status}</td>
                      <td className="px-4 md:px-6 py-4 flex space-x-4">
                        <button
